...
abstract class AbstractScramSHAMechanism extends AbstractMechanism {
    ...
    public ProtonBuffer getInitialResponse() {
        if (state != State.INITIAL) {
            // TODO throw new SaslException("Request for initial response not expected in state " + state);
        }
    ...
    @Override
    public ProtonBuffer getChallengeResponse(final ProtonBuffer challenge) {
        byte[] response;
        ...
        try {
            switch (state) {
                ...
                case CLIENT_PROOF_SENT:
                    ...
                    break;
                default:
                    // TODO throw new SaslException("No challenge expected in state " + state);
                    throw new RuntimeException("No challenge expected in state " + state);
            }
        } catch (Throwable e) {
            throw new RuntimeException(e); // TODO
        }
    ...
    @Override
    public void verifyCompletion() {  // TODO throws SaslException {
...
//        }
    ...
    private byte[] calculateClientProof(final ProtonBuffer challenge) throws SaslException {
        try {
    ...
    private void evaluateOutcome(final ProtonBuffer challenge) throws SaslException {
