    ...
    public void handleAttach(Attach attach, ProtonBuffer payload, int channel, AMQPTestDriver context) {
        ...
        if (response.getPerformative().getTarget() == null && !response.isNullTargetRequired()) {
            if (attach.getTarget() != null) {
                if (attach.getTarget() instanceof org.apache.qpid.proton4j.test.driver.codec.messaging.Target) {
                    org.apache.qpid.proton4j.test.driver.codec.messaging.Target target =
                        (org.apache.qpid.proton4j.test.driver.codec.messaging.Target) attach.getTarget();
                    response.withTarget(target);
                    if (target != null && Boolean.TRUE.equals(target.getDynamic())) {
                        target.setAddress(UUID.randomUUID().toString());
                    }
                } else {
                    org.apache.qpid.proton4j.test.driver.codec.transactions.Coordinator coordinator =
                        (org.apache.qpid.proton4j.test.driver.codec.transactions.Coordinator) attach.getTarget();
